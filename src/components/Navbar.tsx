...
const WA_NUMBER = "596696416727";
const WA_CHOICES = [
  { label: "L'ecole de glisse", message: "Bonjour Airfly, je souhaite des renseignements sur l'ecole de glisse." },
  { label: "Le matos & la boutique", message: "Bonjour Airfly, je souhaite des renseignements sur le materiel et la boutique." },
  { label: "La meteo du spot", message: "Bonjour Airfly, je voudrais des infos sur les conditions meteo du spot." },
];
...
function WhatsAppPopover() {
  ...
  return (
    <div ref={ref} className="relative hidden md:block">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center justify-center w-9 h-9 text-white/70 hover:text-[#25D366] transition-colors duration-200"
        aria-label="WhatsApp"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="absolute right-0 top-12 w-64 bg-white shadow-xl z-[200] overflow-hidden"
            initial={{ opacity: 0, y: -6, scale: 0.97 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: -6, scale: 0.97 }}
            transition={{ duration: 0.18 }}
          >
            <p className="px-4 pt-4 pb-2 text-xs uppercase tracking-widest text-gray-400" style={{ fontFamily: "Mirloanne, serif" }}>
            ...
            {WA_CHOICES.map((c) => (
              <a
                ...
                className="flex items-center justify-between px-4 py-3 text-sm text-gray-700 hover:bg-[#f5f0e8] hover:text-gray-900 transition-colors duration-150 border-t border-gray-100"
          ...
          >
            ...
            {WA_CHOICES.map((c, i) => (
              <motion.div
                ...
                initial={{ opacity: 0, y: 16 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.05 * (links.length + 2 + i), duration: 0.3 }}
              >
                <a
                  href={waLink(c.message)}
                  target="_blank"
