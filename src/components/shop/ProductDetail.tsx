...
export default function ProductDetail({ product, variations }: Props) {
  ...
  // Plus d'etat de chargement ni de cas "introuvable" : les donnees arrivent
  // deja resolues du serveur, et un slug inconnu renvoie un vrai 404 (notFound).
              ...
              {images.length > 1 && (
                <div className="hidden md:flex flex-col gap-2 w-16 shrink-0">
                  {images.map((img, i) => (
                    ...
                    >
                      <Image src={img.src} alt={img.alt || `${product.name} — vue ${i + 1}`} fill className="object-cover" sizes="64px" />
                    </button>
                  ))}
                </div>
              )}
              ...
              <div className="flex md:hidden gap-2 overflow-x-auto pb-1">
                {images.map((img, i) => (
                  <button
                    ...
                    style={{ height: "72px" }}
                  >
                    <Image src={img.src} alt={img.alt || `${product.name} — vue ${i + 1}`} fill className="object-cover" sizes="56px" />
                  </button>
                ))}
              </div>
            )}
            ...
            {images.length > 1 && (
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-50">
                {images.map((img, i) => (
                  ...
                  >
                    <Image src={img.src} alt={img.alt || `${product.name} — vue ${i + 1}`} fill className="object-cover" sizes="48px" />
                  </button>
                ))}
              </div>
            )}
